...
def length_splitter(n:int, step:float, safe_dist:int, override_auto_step=False) -> np.array and int and int:
    ...
    # evt. return n_coordinates
    return step, amount_of_steps


def path(x, y, step_dist, sd):

    x_step_dist, x_amount_of_steps = length_splitter(n=x, step=step_dist, safe_dist=sd, override_auto_step=False)
    y_step_dist, y_amount_of_steps = length_splitter(n=y, step=step_dist, safe_dist=sd, override_auto_step=False)
